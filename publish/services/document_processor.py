...
class DocumentProcessor:
    ...
    def _process_tables(self, doc: Document, terminology: Dict, translation_results: list) -> None:
        ...
        # 处理表格翻译
        for table_idx, table in enumerate(doc.tables, 1):
            logger.info(f"处理表格 {table_idx}/{len(doc.tables)}")
            logger.info(f"表格 {table_idx} 包含 {len(table.rows)} 行")

            for row_idx, row in enumerate(table.rows, 1):
                logger.info(f"处理表格 {table_idx} 第 {row_idx} 行，包含 {len(row.cells)} 个单元格")
                ...
                for cell_idx, cell in enumerate(row.cells, 1):
                    ...
                    if cell_text:
                        # 检查是否已经包含翻译内容（避免重复翻译）
                        if self._is_already_translated(cell_text):
                            logger.info(f"表格 {table_idx} 行 {row_idx} 列 {cell_idx} 已包含翻译内容，跳过: {cell_text[:30]}...")
                            continue
                        ...
                        # 检查单元格中是否包含数学公式
                        text, formulas = self._extract_latex_formulas(cell_text if self.output_format == "bilingual" else original_text)
                        ...
                        try:
                            if self.preprocess_terms:
                                ...
                                if self.source_lang == "zh":
                                    ...
                                    if not cell_terminology:
                                        ...
                                        # 1. 替换术语为占位符
                                        logger.info(f"使用术语预处理方式翻译，找到 {len(cell_terminology)} 个匹配术语")
                                        # 记录术语样本（仅记录前5个术语，避免日志过大）
                                        terms_sample = list(cell_terminology.items())[:5]
                                        logger.info(f"术语样本（前5个）: {terms_sample}")

                                        processed_text = self.term_extractor.replace_terms_with_placeholders(text, cell_terminology)
                                    ...
                                    if not cell_terminology:
                                        ...
                                        try:
                                            ...
                                            # cell_terminology已经是 {外语术语: 中文术语} 格式，直接使用
                                            reverse_terminology = cell_terminology
                            ...
                            logger.error(f"翻译表格 {table_idx} 行 {row_idx} 列 {cell_idx} 失败: {str(e)}")
                            logger.error(f"失败的单元格内容: {cell_text[:100]}...")
                        ...
                        logger.debug(f"表格 {table_idx} 行 {row_idx} 列 {cell_idx} 为空，跳过")
    ...
    def _is_already_translated(self, text: str) -> bool:
        ...
        # 如果文本包含换行符，可能已经是双语对照格式
        if '\n' in text:
            return True

        # 检查是否包含常见的翻译标记
        translation_markers = ['翻译:', 'Translation:', '译文:', '英文:', '中文:', '原文:', 'Original:']
        for marker in translation_markers:
            if marker in text:
                return True

        # 检查是否是明显的双语对照格式（包含括号中的翻译）
        # 例如: "单晶硅(monocrystalline silicon)"
        if re.search(r'[\u4e00-\u9fff]+\s*\([a-zA-Z\s]+\)', text) or re.search(r'[a-zA-Z]+\s*\([\u4e00-\u9fff\s]+\)', text):
            return True
        ...
        if has_chinese and has_english:
            for separator in bilingual_separators:
                if separator in text:
                    # 检查分隔符两边是否分别是中文和英文
                    parts = text.split(separator)
                    if len(parts) == 2:
                        part1_has_chinese = bool(re.search(r'[\u4e00-\u9fff]', parts[0]))
                        part1_has_english = bool(re.search(r'[a-zA-Z]', parts[0]))
                        part2_has_chinese = bool(re.search(r'[\u4e00-\u9fff]', parts[1]))
                        part2_has_english = bool(re.search(r'[a-zA-Z]', parts[1]))

                        # 如果一边主要是中文，另一边主要是英文，可能是双语对照
                        if (part1_has_chinese and not part1_has_english and part2_has_english and not part2_has_chinese) or \
                           (part2_has_chinese and not part2_has_english and part1_has_english and not part1_has_chinese):
                            return True
