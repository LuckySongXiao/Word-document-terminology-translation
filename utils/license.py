    ...
    def verify_license(self, license_code):
        ...
        try:
            ...
            if stored_machine_code != current_machine_code:
                ...
                error_msg = "授权验证失败：\n"
                error_msg += "1. 授权码中的机器码与当前设备不匹配\n"
                error_msg += f"2. 当前设备信息：\n"
                error_msg += f"   - 计算机名：{current_hw.get('hostname', '未知')}\n"
                error_msg += f"   - MAC地址：{':'.join(current_hw.get('mac', '未知').split(':')[-4:]) if current_hw.get('mac') else '未知'}\n"
